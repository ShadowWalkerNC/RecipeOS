/**
 * Core ratio-based scaling engine for RecipeOS.
 * All recipe scaling flows through here — single source of truth.
 */
...
export function scaleIngredients(
  ...
  if (baseServings <= 0 || targetServings <= 0) return ingredients.map((i) => ({ ...i, scaledAmount: i.amount }));
  const factor = targetServings / baseServings;
  return ingredients.map((ing) => {
    const raw = parseFloat(ing.amount);
    if (isNaN(raw)) return { ...ing, scaledAmount: ing.amount };
    const scaled = raw * factor;
    return { ...ing, scaledAmount: formatAmount(scaled) };
  });
}
